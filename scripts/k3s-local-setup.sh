...
ok "CloudNativePG installed in cnpg-system"
...
info "Installing MongoDB Community Operator..."
if helm upgrade --install community-operator mongodb/community-operator \
  ...
  warn "MongoDB operator install timed out or failed. Check: kubectl get pods -n mongodb-operator"
fi
...
echo "    kubectl get pods -n mongodb-operator"
echo ""
