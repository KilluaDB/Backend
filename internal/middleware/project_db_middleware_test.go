...
func TestRequireMongoProject(t *testing.T) {
	projects := mocks.NewProjectStore()
	userID := uuid.New()
	mongo := projects.SeedProject(userID, "mongodb")
	pg := projects.SeedProject(userID, "postgresql")
	...
	r.GET("/projects/:id/x", func(c *gin.Context) {
		c.Set(utils.UserIDContextKey, userID)
		RequireMongoProject(projects)(c)
	}, func(c *gin.Context) { c.Status(http.StatusOK) })
	...
	tests := []struct {
		...
		{"mongo project", mongo.ID.String(), http.StatusOK},
		{"postgres project", pg.ID.String(), http.StatusBadRequest},
		{"unknown project", uuid.New().String(), http.StatusNotFound},
		{"invalid id", "not-uuid", http.StatusBadRequest},
	}
...
// When no authenticated user is in context, the middleware aborts with 401
// before touching the project store.
func TestRequireProjectDBType_Unauthorized(t *testing.T) {
	projects := mocks.NewProjectStore()

	r := gin.New()
	r.GET("/projects/:id/x", func(c *gin.Context) {
		RequirePostgresProject(projects)(c) // no userId set in context
	}, func(c *gin.Context) { c.Status(http.StatusOK) })

	pid := uuid.New().String()
	c, w := testutil.NewGinContext(http.MethodGet, "/projects/"+pid+"/x", nil, nil)
