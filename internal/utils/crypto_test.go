...
func TestDecryptString_MissingKey(t *testing.T) {
	os.Unsetenv("DB_CRED_ENCRYPTION_KEY")
	_, err := DecryptString("anything")
	assert.ErrorIs(t, err, ErrMissingEncryptionKey)
}

func TestDecryptString_Errors(t *testing.T) {
	t.Setenv("DB_CRED_ENCRYPTION_KEY", "01234567890123456789012345678901")
	...
	tests := []struct {
		...
		// Valid base64 but fewer bytes than the GCM nonce size.
		{"too short", base64.StdEncoding.EncodeToString([]byte("short"))},
		// Valid base64, long enough for a nonce, but not authentic ciphertext.
		{"open fails", base64.StdEncoding.EncodeToString(make([]byte, 64))},
	}
