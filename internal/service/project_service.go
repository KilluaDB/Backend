...
func (s *ProjectService) GetProjectByIDAndUserID(ctx context.Context, projectID, userID string) (*model.Project, error) {
	projectUUID, err := utils.ParseUUID(projectID)
	if err != nil {
		return nil, fmt.Errorf("%w: %v", ErrInvalidProjectID, err)
	}
...
func (s *ProjectService) GetProjectsByUserID(ctx context.Context, userID string) ([]model.Project, error) {
	userUUID, err := utils.ParseUUID(userID)
	if err != nil {
		return nil, fmt.Errorf("%w: %v", ErrInvalidUserID, err)
	}
