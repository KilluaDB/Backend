...
func TestTableRepository_ListIndexes_queryError(t *testing.T) {
	mock := newRepoMock(t)
	mock.ExpectQuery(`(?s)SELECT i\.relname.*pg_class t`).
		WithArgs("public", "users").
...
func TestTableRepository_ListIndexes_scanError(t *testing.T) {
	mock := newRepoMock(t)
	mock.ExpectQuery(`(?s)SELECT i\.relname.*pg_class t`).
		WithArgs("public", "users").
		WillReturnRows(pgxmock.NewRows([]string{
...
func TestTableRepository_DropIndex_queryError(t *testing.T) {
	mock := newRepoMock(t)
	mock.ExpectQuery(`(?s)SELECT ix\.indisprimary`).
		WithArgs("public", "users", "idx").
		WillReturnError(errors.New("generic query error"))

	repo := NewTableRepository()
	err := repo.DropIndex(context.Background(), mock, "public", "users", "idx")
	require.Error(t, err)
	// Not ErrIndexNotFound or ErrCannotDropPrimaryIndex — generic error propagates.
	assert.Contains(t, err.Error(), "generic query error")
}

func TestTableRepository_DropIndex_execError(t *testing.T) {
	mock := newRepoMock(t)
	mock.ExpectQuery(`(?s)SELECT ix\.indisprimary`).
		WithArgs("public", "users", "idx").
		WillReturnRows(pgxmock.NewRows([]string{"indisprimary"}).AddRow(false))
	mock.ExpectExec(`DROP INDEX "public"\."idx"`).
