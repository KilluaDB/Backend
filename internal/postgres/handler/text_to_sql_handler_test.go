...
type stubDSNProvider struct {
	dsn string
	err error
}

func (s *stubDSNProvider) GetConnectionDSN(ctx context.Context, userID, projectID uuid.UUID) (string, uuid.UUID, error) {
	if s.err != nil {
...
var _ infra.DSNProvider = (*stubDSNProvider)(nil)
...
func TestTextToSQLHandler_GenerateAndExecuteSQL(t *testing.T) {
	...
	t.Run("ErrNoDBCredentials", func(t *testing.T) {
		svc := &mockTextToSQLService{
			err: service.ErrNoDBCredentials,
		}
		h := NewTextToSQLHandler(svc, nil)
		body := map[string]string{"question": "list users"}
		c, w := textToSQLCtx(http.MethodPost, "/", uuid.New(), uuid.New().String(), body)
	...
	t.Run("default error", func(t *testing.T) {
		svc := &mockTextToSQLService{
			err: errors.New("unexpected service failure"),
		}
		h := NewTextToSQLHandler(svc, nil)
		body := map[string]string{"question": "list users"}
		c, w := textToSQLCtx(http.MethodPost, "/", uuid.New(), uuid.New().String(), body)
	...
	t.Run("generation failed", func(t *testing.T) {
		...
		assert.Contains(t, resp["message"], "SQL generation failed")
	})

	t.Run("invalid body", func(t *testing.T) {
		svc := &mockTextToSQLService{}
		h := NewTextToSQLHandler(svc, nil)
		c, w := textToSQLCtx(http.MethodPost, "/", uuid.New(), uuid.New().String(), "not-json")
		h.GenerateAndExecuteSQL(c)
		assert.Equal(t, http.StatusBadRequest, w.Code)
	})
	...
	t.Run("upstream success", func(t *testing.T) {
		ts := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
			json.NewEncoder(w).Encode(service.TextToSQLResponse{
				Success: true,
				SQL:     "SELECT 1",
			})
		}))
		t.Cleanup(ts.Close)
		t.Setenv("TEXT_TO_SQL", ts.URL)

		projects := mocks.NewProjectStore()
		userID := uuid.New()
		proj := projects.SeedProject(userID, "postgresql")

		tts := service.NewTextToSQLService(&stubDSNProvider{}, projects)
		pool := newTextToSQLPool(t)
		pool.ExpectQuery("SELECT 1").WillReturnRows(
			pgxmock.NewRows([]string{"?column?"}).AddRow(int32(1)),
		)
		qsvc := service.NewQueryService(nil, 50)
		qsvc.SetRunnerSourceForTest(&staticTextToSQLRunnerSource{runner: pool})

		h := NewTextToSQLHandler(tts, qsvc)
		body := map[string]string{"question": "list users"}
		c, w := textToSQLCtx(http.MethodPost, "/", userID, proj.ID.String(), body)
		h.GenerateAndExecuteSQL(c)
		assert.Equal(t, http.StatusOK, w.Code)
