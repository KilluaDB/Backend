...
func TestQueryHandler_ExecuteQuery_notRunning(t *testing.T) {
	svc := pgservice.NewQueryService(nil, 50)
	svc.SetRunnerSourceForTest(staticRunnerSource{err: service.ErrNoRunningInstance})
	h := NewQueryHandler(svc)
...
func TestQueryHandler_ExecuteQuery_runnerError(t *testing.T) {
	svc := pgservice.NewQueryService(nil, 50)
	svc.SetRunnerSourceForTest(staticRunnerSource{err: errors.New("connection failed")})
	h := NewQueryHandler(svc)
...
func TestQueryHandler_GetQueryHistory_invalidProjectID(t *testing.T) {
	h := NewQueryHandler(pgservice.NewQueryService(nil, 50))
	c, w := testutil.NewGinContext(http.MethodGet, "/projects/bad/query/history", nil, nil)
	c.Set(utils.UserIDContextKey, uuid.New())
	c.Params = gin.Params{{Key: "id", Value: "not-a-uuid"}}
...
func TestQueryHandler_GetQueryHistory_success(t *testing.T) {
	...
	mock.ExpectQuery(`SELECT EXISTS.*pg_stat_statements`).
		WillReturnRows(pgxmock.NewRows([]string{"exists"}).AddRow(true))
	mock.ExpectQuery(`(?s)SELECT query.*FROM pg_stat_statements.*LIMIT 50`).
		WillReturnRows(pgxmock.NewRows([]string{"query", "calls", "total_exec_time", "mean_exec_time", "rows", "shared_blks_hit", "shared_blks_read", "temp_blks_written"}).
			AddRow("SELECT 1", int64(10), 100.0, 10.0, int64(100), int64(1000), int64(10), int64(0)).
			AddRow("SELECT * FROM users", int64(5), 50.0, 10.0, int64(50), int64(500), int64(5), int64(0)))
