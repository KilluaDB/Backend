...
// mockSchemaPoolSource injects a mock pool at the schemaPool seam for tests.
type mockSchemaPoolSource struct {
	pool schemaPoolRunner
	err  error
}

func (m mockSchemaPoolSource) SchemaPool(ctx context.Context, userID, projectID uuid.UUID) (schemaPoolRunner, error) {
	if m.err != nil {
		return nil, m.err
	}
	return m.pool, nil
...
func TestSchemaService_VisualizeSchema(t *testing.T) {
	...
	t.Run("success returns mermaid diagram", func(t *testing.T) {
		mock := newSchemaServiceMockPool(t)
		tables := []model.Table{
			tableFixture("users", []model.Column{col("id", "uuid")}, []string{"id"}, nil),
		}
...
func TestSchemaService_ListSchemas(t *testing.T) {
	...
	t.Run("GetPool fails returns error", func(t *testing.T) {
		svc := NewSchemaService(mockSchemaInstanceConn{err: errors.New("pool error")}, nil)
		_, err := svc.ListSchemas(ctx, uuid.New(), uuid.New())
		require.Error(t, err)
		assert.Contains(t, err.Error(), "pool error")
	})
	...
	t.Run("query error propagates", func(t *testing.T) {
		...
		mock.ExpectQuery(`(?s)SELECT schema_name.*information_schema\.schemata`).
			WillReturnError(errors.New("schemata query failed"))
		...
		_, err := svc.ListSchemas(ctx, uuid.New(), uuid.New())
		require.Error(t, err)
		assert.Contains(t, err.Error(), "schemata query failed")
	})

	t.Run("success returns schema names", func(t *testing.T) {
		mock := newSchemaServiceMockPool(t)
		mock.ExpectQuery(`(?s)SELECT schema_name.*information_schema\.schemata`).
		...
		got, err := svc.ListSchemas(ctx, uuid.New(), uuid.New())
		require.NoError(t, err)
		assert.Equal(t, []string{"public", "app"}, got)
	})
}

func TestSimplifyDataType(t *testing.T) {
	tests := []struct {
		dataType string
		want     string
	}{
	...
	for _, tt := range tests {
		t.Run(tt.dataType, func(t *testing.T) {
			assert.Equal(t, tt.want, simplifyDataType(tt.dataType))
		})
	}
}
