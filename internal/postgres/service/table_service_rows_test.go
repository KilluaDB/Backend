...
func TestTableService_DeleteRowsByFilter_withFilter(t *testing.T) {
	mock := newTableRowsMockPool(t)
	mock.ExpectExec(`DELETE FROM "public"\."users" WHERE "email" = \$1`).
		WithArgs("x@test.com").
		WillReturnResult(pgxmock.NewResult("DELETE", 1))

	svc := tableSvcWithRowsMock(t, mock)
	err := svc.DeleteRowsByFilter(context.Background(), uuid.New(), uuid.New(), "public", "users",
		map[string]interface{}{"email": "x@test.com"})
	require.NoError(t, err)
}

func TestTableService_DeleteRowsByFilter_invalidFilterColumn(t *testing.T) {
	svc := tableSvcWithRowsMock(t, newTableRowsMockPool(t))
	err := svc.DeleteRowsByFilter(context.Background(), uuid.New(), uuid.New(), "public", "users",
		map[string]interface{}{"bad col": 1})
	require.Error(t, err)
}

func TestTableService_DeleteRowsByFilter_poolError(t *testing.T) {
	svc := NewTableService(stubInstanceConn{}, repository.NewTableRepository())
