...
func TestQueryService_ValidateSQLQuery(t *testing.T) {
	...
	tests := []struct {
		...
		{"drop schema", "DROP SCHEMA public", "not allowed"},
		{"create database", "CREATE DATABASE x", "not allowed"},
		{"create schema", "CREATE SCHEMA x", "not allowed"},
		{"alter database", "ALTER DATABASE x SET name = y", "not allowed"},
		...
		{"multi-stmt semicolons", "SELECT 1; SELECT 2; SELECT 3", "multiple statements"},
		{"multi-stmt with trailing semi", "SELECT 1; ", ""},
	}
...
func TestQueryService_SetRunnerSourceForTest(t *testing.T) {
	t.Run("nil clears runnerSource", func(t *testing.T) {
		svc := NewQueryService(nil, 50)
		svc.SetRunnerSourceForTest(nil)
		assert.Nil(t, svc.runnerSource)
	})

	t.Run("non-nil wraps in bridge", func(t *testing.T) {
		mock := newMockPool(t)

		// Anonymous QueryRunnerSource adapter.
		src := &queryRunnerSrcAdapter{runner: mock, id: uuid.New()}
		svc := NewQueryService(nil, 50)
		svc.SetRunnerSourceForTest(src)
		require.NotNil(t, svc.runnerSource)

		mock.ExpectQuery(regexp.QuoteMeta("SELECT 1")).
			WillReturnRows(pgxmock.NewRows([]string{"?"}).AddRow(int64(1)))
		_, _, err := svc.ExecuteSQLQuery(context.Background(), uuid.New(),
			&ExecuteQueryRequest{Query: "SELECT 1"}, uuid.New())
		require.NoError(t, err)
	})
}

// queryRunnerSrcAdapter implements QueryRunnerSource (exported) for SetRunnerSourceForTest tests.
type queryRunnerSrcAdapter struct {
	runner pgxmock.PgxPoolIface
	id     uuid.UUID
}

func (a *queryRunnerSrcAdapter) QueryRunner(ctx context.Context, userID, projectID uuid.UUID) (QueryRunner, uuid.UUID, error) {
	return a.runner, a.id, nil
}

func TestQueryService_ExecuteSQLQuery_poolForQueryError(t *testing.T) {
	// runnerSource=nil → falls through to instanceConn.GetPoolWithMeta → error.
	svc := NewQueryService(stubInstanceConn{}, 50)
	_, _, err := svc.ExecuteSQLQuery(context.Background(), uuid.New(),
		&ExecuteQueryRequest{Query: "SELECT 1"}, uuid.New())
	require.Error(t, err)
	assert.Contains(t, err.Error(), "unexpected GetPoolWithMeta")
}

func TestQueryService_ExecuteSQLQuery_poolForQueryRunnerSourceError(t *testing.T) {
	svc := NewQueryService(nil, 50)
	svc.runnerSource = mockRunnerSource{err: errors.New("runner unavailable")}
	_, _, err := svc.ExecuteSQLQuery(context.Background(), uuid.New(),
		&ExecuteQueryRequest{Query: "SELECT 1"}, uuid.New())
	require.Error(t, err)
	assert.Contains(t, err.Error(), "runner unavailable")
}

func TestQueryService_GetQueryHistory_poolForHistoryError(t *testing.T) {
	// runnerSource=nil → falls through to instanceConn.GetPool → error.
	svc := NewQueryService(stubInstanceConn{}, 50)
	_, err := svc.GetQueryHistory(context.Background(), uuid.New(), uuid.New(), 10)
	require.Error(t, err)
	assert.Contains(t, err.Error(), "unexpected GetPool")
}

func TestQueryService_ExecuteSQLQuery_selectMixedTypes(t *testing.T) {
	mock := newMockPool(t)
	svc := svcWithMock(t, mock, uuid.New())
	query := "SELECT id, data, uuid_col, ts, nil_col FROM t"

	uid := uuid.New()
	mock.ExpectQuery(regexp.QuoteMeta(query)).
		WillReturnRows(pgxmock.NewRows([]string{"id", "data", "uuid_col", "ts", "nil_col"}).
			AddRow(int64(1), []byte("hello"), [16]byte(uid),
				time.Date(2024, 1, 2, 0, 0, 0, 0, time.UTC), nil))

	result, _, err := svc.ExecuteSQLQuery(context.Background(), uuid.New(),
		&ExecuteQueryRequest{Query: query}, uuid.New())
	require.NoError(t, err)
	require.Len(t, result.Rows, 1)
	assert.Equal(t, "hello", result.Rows[0]["data"])
	assert.Equal(t, uid.String(), result.Rows[0]["uuid_col"])
	assert.Equal(t, "2024-01-02T00:00:00Z", result.Rows[0]["ts"])
	assert.Nil(t, result.Rows[0]["nil_col"])
}

func TestQueryService_ExecuteSQLQuery_selectValuesError(t *testing.T) {
	mock := newMockPool(t)
	svc := svcWithMock(t, mock, uuid.New())
	...
	mock.ExpectQuery(regexp.QuoteMeta(query)).
		WillReturnRows(pgxmock.NewRows([]string{"id"}).
			AddRow(int64(1)).
			RowError(0, errors.New("row value error")))

	result, _, err := svc.ExecuteSQLQuery(context.Background(), uuid.New(),
		&ExecuteQueryRequest{Query: query}, uuid.New())
	require.Error(t, err)
	require.NotNil(t, result)
	assert.Contains(t, result.Error, "row value error")
}

func TestQueryService_ExecuteSQLQuery_selectRowsErr(t *testing.T) {
	mock := newMockPool(t)
	svc := svcWithMock(t, mock, uuid.New())
	query := "SELECT id FROM t"
	...
	mock.ExpectQuery(regexp.QuoteMeta(query)).
		WillReturnRows(pgxmock.NewRows([]string{"id"}).
			AddRow(int64(1)).
			CloseError(errors.New("rows iterator error")))

	result, _, err := svc.ExecuteSQLQuery(context.Background(), uuid.New(),
		&ExecuteQueryRequest{Query: query}, uuid.New())
	require.Error(t, err)
	require.NotNil(t, result)
	assert.Contains(t, result.Error, "rows iterator error")
}

func TestQueryService_GetQueryHistory_queryError(t *testing.T) {
	mock := newMockPool(t)
	svc := svcWithMock(t, mock, uuid.New())
	...
	`
	mock.ExpectQuery(regexp.QuoteMeta(historySQL)).
		WillReturnError(errors.New("history query failed"))

	_, err := svc.GetQueryHistory(context.Background(), uuid.New(), uuid.New(), 10)
	require.Error(t, err)
	assert.Contains(t, err.Error(), "history query failed")
}

func TestQueryService_GetQueryHistory_scanError(t *testing.T) {
	mock := newMockPool(t)
	svc := svcWithMock(t, mock, uuid.New())
	...
	mock.ExpectQuery(regexp.QuoteMeta(historySQL)).
		WillReturnRows(pgxmock.NewRows([]string{
			...
			AddRow("SELECT 1", int64(1), 1.0, 1.0, int64(1), int64(0), int64(0), int64(0)).
