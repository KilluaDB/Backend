...
func NewGoogleAuthHandler(googleAuthService *service.GoogleAuthService, oauthConfig *oauth2.Config) *GoogleAuthHandler {
...
func (h *GoogleAuthHandler) Login(c *gin.Context) {
	...
	*/
	...
	if err != nil {
		response.Fail(c, http.StatusInternalServerError, err, "Failed to generate state")
	}
...
func (h *GoogleAuthHandler) Callback(c *gin.Context) {
	...
	if queryState == "" {
		response.Fail(c, http.StatusBadRequest, nil, "Missing state parameter")
		return
	}

	cookieState, err := c.Cookie("oauth_state")
	if err != nil {
		response.Fail(c, http.StatusBadRequest, err, "Missing state cookie")
		return
	}

	if queryState != cookieState {
		response.Fail(c, http.StatusForbidden, nil, "State mismatch - possible CSRF attack")
		return
	}
	...
	code := c.Query("code")
	if code == "" {
		response.Fail(c, http.StatusBadRequest, nil, "Missing code")
		return
	}
	...
	if err != nil {
		response.Fail(c, http.StatusInternalServerError, err, "Token exchange failed")
		return
	}
	...
	if err != nil {
		response.Fail(c, http.StatusInternalServerError, err, "Failed to login")
		return
	}
	...
	response.Success(c, http.StatusOK, res, "User Login Successfully!")
}
