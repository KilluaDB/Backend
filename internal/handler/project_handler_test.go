...
type projectStoreFailingCreate struct {
	repository.ProjectStore
}

func (s *projectStoreFailingCreate) Create(ctx context.Context, project *model.Project) error {
	return errors.New("db failure")
}

type projectStoreFailingGetByUserID struct {
	repository.ProjectStore
}

func (s *projectStoreFailingGetByUserID) GetByUserID(ctx context.Context, userID uuid.UUID) ([]model.Project, error) {
	return nil, errors.New("db failure")
}

type projectStoreFailingGetByIDAndUserID struct {
	repository.ProjectStore
}

func (s *projectStoreFailingGetByIDAndUserID) GetByIDAndUserID(ctx context.Context, id, userID uuid.UUID) (*model.Project, error) {
	return nil, errors.New("db failure")
}
...
func TestProjectHandler_GetProject_serviceError(t *testing.T) {
	userID := uuid.New()
	projects := mocks.NewProjectStore()
	svc := service.NewProjectService(&projectStoreFailingGetByIDAndUserID{ProjectStore: projects}, &servicetest.Provisioner{}, nil, nil)
	h := NewProjectHandler(svc)
	r := projectRouter(h, userID)

	w := httptest.NewRecorder()
	r.ServeHTTP(w, httptest.NewRequest(http.MethodGet, "/projects/"+uuid.New().String(), nil))
...
func TestProjectHandler_CreateProject_serviceError(t *testing.T) {
	userID := uuid.New()
	projects := mocks.NewProjectStore()
	svc := service.NewProjectService(&projectStoreFailingCreate{ProjectStore: projects}, &servicetest.Provisioner{}, nil, nil)
	h := NewProjectHandler(svc)
	r := projectRouter(h, userID)

	c, w := testutil.NewGinContext(http.MethodPost, "/projects", map[string]string{
		"name": "test", "db_type": "postgres", "password": "SecurePass123!",
	}, nil)
	c.Set(utils.UserIDContextKey, userID)
	r.HandleContext(c)
	assert.Equal(t, http.StatusInternalServerError, w.Code)
...
func TestProjectHandler_ListProjects_serviceError(t *testing.T) {
	userID := uuid.New()
	projects := mocks.NewProjectStore()
	svc := service.NewProjectService(&projectStoreFailingGetByUserID{ProjectStore: projects}, &servicetest.Provisioner{}, nil, nil)
	h := NewProjectHandler(svc)
	r := projectRouter(h, userID)
...
func TestProjectHandler_GetProjectAccess_unauthorized(t *testing.T) {
	h := newProjectHandler(mocks.NewProjectStore(), &servicetest.Provisioner{})
	r := projectRouter(h, uuid.Nil)

	c, w := testutil.NewGinContext(http.MethodGet, "/projects/"+uuid.New().String()+"/access", nil, nil)
	c.Params = gin.Params{{Key: "id", Value: uuid.New().String()}}
	r.HandleContext(c)
	assert.Equal(t, http.StatusUnauthorized, w.Code)
}

func TestProjectHandler_GetProjectAccess_notFound(t *testing.T) {
	userID := uuid.New()
	projects := mocks.NewProjectStore()
	prov := &servicetest.Provisioner{
		External: true,
...
func TestProjectHandler_DeleteProject_serviceError(t *testing.T) {
	userID := uuid.New()
	projects := mocks.NewProjectStore()
	svc := service.NewProjectService(&projectStoreFailingGetByIDAndUserID{ProjectStore: projects}, &servicetest.Provisioner{}, nil, nil)
	h := NewProjectHandler(svc)
