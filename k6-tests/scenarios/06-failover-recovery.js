...
const reconnectTime = new Trend("failover_reconnect_time_ms", true);
const errorWindow = new Counter("failover_error_window_ms");
...
export function setup() {
   ...
   const pg = createProject(auth.access_token, "k6-failover-pg", "sql");
   let pgR = null;
   if (pg) pgR = waitForProjectReady(auth.access_token, pg.id);
   if (pgR) {
      execSQL(auth.access_token, pg.id, `CREATE TABLE IF NOT EXISTS k6_failover(id SERIAL PRIMARY KEY,ts TIMESTAMPTZ DEFAULT NOW())`);
      for (let i = 0;
         i < 50;
         i++)execSQL(auth.access_token, pg.id, `INSERT INTO k6_failover DEFAULT VALUES`);

   } return { token: auth.access_token, pgId: pg ? pg.id : null, pgR: !!pgR, podKilled: false, killTime: 0 };
}
...
export default function (d) {
   ...
   const res = execSQL(d.token, d.pgId, `SELECT COUNT(*) FROM k6_failover`);
   const elapsed = Date.now() - start;

   const ok = check(res, { "failover query ok": (r) => r.status === 200 });
   failoverErrorRate.add(!ok);
   if (!ok) {
      failoverErrors.add(1);
      errorWindow.add(elapsed);
      ...
      for (let attempt = 0;
         ...
         attempt++) {
            sleep(1);
         const retry = execSQL(d.token, d.pgId, `SELECT 1`);
         if (retry.status === 200) {
            reconnected = true;
            reconnectTime.add(Date.now() - reconStart);
            break;
         }
      } if (!reconnected) reconnectTime.add(30000);
   } sleep(0.5);
}
